...
/************ SEARCH ***********/
$con = connectTo();
$exists = $con->query("select * from `bi8ojyxk04q87n1nvwba`.`teacher` where email = '$email'");
if($exists && $con->affected_rows) {
  $con->close();
  respond("error","exists");
} 
/*********** INSERT ************/
$addTeacher = $con->query("insert into `bi8ojyxk04q87n1nvwba`.`teacher` (`name`, `email`, `phone`, `password`) values ('$name', '$email', '$phone', '$pass')");
